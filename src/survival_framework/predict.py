...
from survival_framework.utils import ensure_dir, default_time_grid
...
def generate_predictions(
    ...
    """Generate survival probability predictions for all records in input CSV.
    ...
    Args:
        csv_path: Path to input CSV file with same schema as training data
        output_dir: Directory to save prediction CSV. Defaults to "data/predictions"
        time_horizons: List of time points (months) for survival predictions.
            Defaults to [3, 6, 12, 18, 24, 36] if None
        artifacts_dir: Directory containing model_summary.csv
    ...
    Side Effects:
        Creates output_dir if it doesn't exist and writes predictions CSV with columns:
        - account_entities_key: Account identifier
        - model_name: Name of the model used
        - expected_survival_months: Expected survival time (restricted mean, up to 120 months)
        - survival_prob_Xm: Survival probability at X months (one column per horizon)

    Example:
        >>> pred_path = generate_predictions("data/sample/survival_inputs_sample2000.csv")
        >>> print(f"Predictions saved to: {pred_path}")
        Predictions saved to: data/predictions/survival_predictions_20250123_143052.csv
    ...
    # Load best model
    model_name, pipeline = load_best_model(artifacts_dir, models_dir)
    ...
    output_filename = f"survival_predictions_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv"
